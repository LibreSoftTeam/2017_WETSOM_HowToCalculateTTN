...
%Eyolfson \emph{et al.} ~\cite{eyolfson2011time} We define a bug’s lifetime to be the time from the earliest commit which introduced the bugto the bug-fixing commit, also they find that the time and day of a code commit may affect the quality of the code.
...
Table~\ref{table} \gema{all the references to tables say Table V instead of their correct number, I don't know what is wrong} shows the mean of the Time To Fix (TTF) and the Time To Notify (TTN) variables computed in each project using the exactly BIC. The table also shows, the value of the TTN calculated through the use of SZZ using the nearest (oTTN) and further (pTTN) suspicious commit to insert the bug.  
