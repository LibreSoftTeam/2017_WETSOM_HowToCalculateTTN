...
We have analyzed exactly the values of the TTN, the EuBIC, and the TTF in the 76 bug fixing commits, 39 belong to Nova and 37 belong to ElasticSearch. In addition, we compare the real TTN value with the optimistic (oTTN) and pessimistic (pTTN) values resulting of SZZ.

Table~\ref{table} \gema{all the references to tables say Table V instead of their correct number, I don't know what is wrong} shows the mean of the Time To Fix (TTF) and the Time To Notify (TTN) variables computed in each project using the exactly BIC. The table also shows, the value of the TTN calculated through the use of SZZ using the nearest (oTTN) and furthest (pTTN) suspicious commit to insert the bug.  
...
\textbf{$H_0$: The real values of the Time to Notify (TTN) a bug in both projects differ from the values calculated with SZZ } 
\end{minipage}}
\vspace{0.1cm}
...
\vspace{0.2cm}
\fbox{\begin{minipage}{24em}
\textbf{ $H_2$: The experience of the author who inserted the bug in the source code and the Time To Notify such bug have a correlation in Nova project} 
\end{minipage}}
\vspace{0.1cm}
