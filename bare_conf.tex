...
Our main findings reveal that the \emph{real} mean TTN ranges from one month in ElasticSearch to three months in Nova \grex{How different is it from the way it is done with SZZ?}. On the other hand, only one of the projects showed a weak correlation between TTN and the experience of the author who introduced the bug.
...
\begin{enumerate}
		...
		\item Analyze which one of the previous commits was the BIC. In the case where none of them was the cause of the bug, track back each previous commit until the line causing the failure can be found in some of the previous commits. \grex{What happens if it is a fixing commit that only adds lines?}
