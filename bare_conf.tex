...
%\gema{Review 1 said that the paper needs writing some examples or scenarios where TTN could be useful--- In my opinion the above paragraph is an example of where TTN could be useful----}
...
%The kind of modification done in the code such as API changes, variable renaming or optimization might increase or dicrease the value of TTN. Thus, this value could be an indicator of the difficulty to discover the bug where the RQ1 may be an estimation about how much time (in mean) it takes to notify the error inserted in the code depending on the kind of modification.  In addition, RQ2 shows how strongly the pairs of variables are related to predict which changes should be reviewed carefully and in detail depending on the author that committed them or other characteristics presented in the project.


%\gema{Review 1 said that we should clarify how important are RQ1 and RQ2 for researchers and practitioners----- Done in the above paragraph!!------------ } \grex{I think the reviewer is asking for takeaways, on a much more higher level. What you've done is rephrasing the RQs}\gema{done in the above paragraph}
...
%\gema{Review 1 said that we need to write how bug reports and fixing commits are linked, maybe with an additional step in figure~\ref{fig:methodology} ----- solved in the description of STEP 1 ----}
\begin{figure}[ht]
\centering
...
%\gema{Review 1 said that the comparison with SZZ should be mentioned earlier in the paper (goal/contribution)---- Fixed in the introduction, column 2 paragraph 3----}
...
%\gema{Review 1 said that we should detail the research questions, describing the statistical tests used to answer this questions --- Done at the end of this section!!! ------ }
...
%\gema{One reviewer said that this sentence is not understandable: "Our study also shows a negative correlation between the TTN and EuBIC, which may indicates that the higher time to notify, the fewer experience of the author who created the bug" ---- DONE in the next sentence --------}.
...
Another future line could be to perform a detailed research on how this metric could be helpful in defect prediction field, building new models or improving them. %\grex{defect prediction filed?}
...
%\gema{Review 3 said that we should elaborate more the future work---DONE in the last 2 paragraphs at this section ---! }
