...
As results, we have observed that the mean of the TTN in the projects was 312 days and 431 days. However, only one of the projects showed a moderate correlation between the experience of the author who created the bug and TTN.
...
\begin{itemize}
\item $H_0$: The real values of TTN do not differ from the values calculated using SZZ.
\item $H_1$: There is not correlation between the Time To Notify and the Time To Fix.
\item $H_2$: There is not correlation between the Time To Notify and the experience of the developer inserting the bug.
\item $H_3$: There is not correlation between the Time To Fix and the experience of the developer inserting the bug
\end{itemize}
...
After analyzing several fixing commits in this study, we have realized that the \emph{real} Time To Notify a bug differs to the one computed using an algorithm such as SZZ, the percentage of times that this metric concurs is 36\% in Nova and 24\% in ElasticSearch. The TTN computed using SZZ is almost the half that the real one calculated. The principal reasons of this results may be (1) The bug fixing commit with only new lines are discarded by SZZ and, (2) The wrong assumption done by SZZ, where the earliest commit to the bug fixing commit is the suspicious to be the BIC.
...
On the contrary, the results obtained denote that the hypothesis $H_1$ and $H_3$ are positives. 
